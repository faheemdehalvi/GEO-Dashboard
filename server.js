...
app.post('/api/snipe/revamp', async (req, res) => {
  ...
  // Time budget is tight on this endpoint because the snipe's draft can
  // be 30k+ chars of input, which pushes the AI generation time toward
  // the Vercel 300s cap. Caps below are tuned to leave AI ~170s.
  send({ type: 'progress', stage: 'sitemap', message: 'Fetching sitemap...' });
  const sitemapResults = await Promise.all(sitemapUrls.map(u => fetchSitemapUrls(u)));
  const rawSitemapUrls = sitemapResults.flat();

  send({ type: 'progress', stage: 'sitemap', message: `Validating ${Math.min(30, rawSitemapUrls.length)} sitemap URLs...` });
  const allInternalUrls = await prefilterSitemap(rawSitemapUrls, { cap: 30, concurrency: 15, timeoutMs: 3000 });

  // Phase 2: verify external links INSIDE the snipe's draft.
  const internalDomain = brand.domain.replace(/^www\./,'').toLowerCase();
  const allLinks = extractMarkdownLinks(sourceDraft, internalDomain);
  const externalLinksInDraft = allLinks.filter(l => !l.internal).slice(0, 6);
  ...
  // Cap the draft input to keep total prompt size manageable. 25k chars is
  // ~6k input tokens — plenty for a full article revamp without burning
  // generation budget on rehashing every word.
  const draftForPrompt = sourceDraft.length > 25000 ? sourceDraft.slice(0, 25000) + '\n\n[...truncated for length...]' : sourceDraft;

  const userPrompt = `You are revamping an existing draft article for ${brand.name} that was generated by the Snipe process. The original draft is below — apply the Revamp skill's full Phase 1-3 review + Mode ${modeNum} treatment to it.
